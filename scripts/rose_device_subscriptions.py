...
def build_computer_profile(proxies: list[dict], device_uuid: str) -> dict:
    ...
    provider_specs = {
        "MIHOMO-Apple-Intelligence": "apple-intelligence.list",
        "MIHOMO-Telegram": "telegram.list",
        "MIHOMO-Crypto": "crypto.list",
        "MIHOMO-Proxy": "proxy.list",
        "MIHOMO-CDN": "cdn.list",
        "MIHOMO-Speedtest": "speedtest.list",
        "MIHOMO-APNs": "apns.list",
        "MIHOMO-Apple": "apple.list",
        "MIHOMO-Microsoft": "microsoft.list",
        "MIHOMO-Direct": "direct.list",
    }
    profile["rule-providers"] = {
        name: {
            ...
            "behavior": "classical",
            "format": "text",
            "url": f"{mihomo_base}/{filename}",
            "path": f"./ruleset/rose/{filename}",
            "interval": 86400,
            "proxy": "COMPUTER_DEFAULT",
        }
        for name, filename in provider_specs.items()
    }
    profile["rule-providers"]["AWAvenue-Ads"] = {
        "type": "http",
        "behavior": "classical",
        "format": "yaml",
        "url": (
            "https://raw.githubusercontent.com/TG-Twilight/AWAvenue-Ads-Rule/main/"
            "Filters/AWAvenue-Ads-Rule-Clash-Classical.yaml"
        ),
        "path": "./ruleset/rose/AWAvenue-Ads.yaml",
        "interval": 86400,
        "proxy": "COMPUTER_DEFAULT",
    }
    ...
    profile["rules"] = [
        ...
        *(f"DOMAIN-SUFFIX,{domain},COMPUTER_VIDEO" for domain in video_domains),
        "RULE-SET,MIHOMO-Apple-Intelligence,COMPUTER_AI",
        "RULE-SET,MIHOMO-Telegram,COMPUTER_SOCIAL",
        "RULE-SET,MIHOMO-Crypto,COMPUTER_DEFAULT",
        "RULE-SET,MIHOMO-Proxy,COMPUTER_DEFAULT",
        "RULE-SET,MIHOMO-CDN,COMPUTER_DEFAULT",
        "RULE-SET,MIHOMO-Speedtest,COMPUTER_DEFAULT",
        "RULE-SET,MIHOMO-APNs,DIRECT",
        "RULE-SET,MIHOMO-Apple,DIRECT",
        "RULE-SET,MIHOMO-Microsoft,DIRECT",
        "RULE-SET,MIHOMO-Direct,DIRECT",
        "GEOIP,CN,DIRECT",
        "MATCH,PROXY",
    ]
    return profile
...
def generate_outputs(env: dict[str, str], master_root: Path) -> dict[tuple[str, str, str], str]:
    ...
    return {
        ...
        ("computer", "clashMetaProfiles", env["COMPUTER_TOKEN"]): yaml.safe_dump(
            build_computer_profile(proxies, env["COMPUTER_UUID"]),
            allow_unicode=True,
            sort_keys=False,
            width=1000,
        ),
