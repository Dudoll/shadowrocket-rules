...
def test_computer_profile_uses_remote_mihomo_and_awavenue_rule_providers() -> None:
    names = [
        "DMIT_CF_WS_443",
        "BAND_CF_WS_443",
        "DMIT_REALITY_IPv4_443",
    ...
    assert providers["MIHOMO-Crypto"] == {
        "type": "http",
        "behavior": "classical",
        "format": "text",
        "url": "https://raw.githubusercontent.com/HenryChiao/MIHOMO_YAMLS/main/custom/crypto.list",
        "path": "./ruleset/rose/crypto.list",
        "interval": 86400,
        "proxy": "COMPUTER_DEFAULT",
    }
    assert providers["AWAvenue-Ads"]["behavior"] == "classical"
    assert providers["AWAvenue-Ads"]["format"] == "yaml"
    assert providers["AWAvenue-Ads"]["proxy"] == "COMPUTER_DEFAULT"
    assert providers["AWAvenue-Ads"]["url"].endswith("AWAvenue-Ads-Rule-Clash-Classical.yaml")
    assert rules[0] == "RULE-SET,AWAvenue-Ads,REJECT"
    assert "RULE-SET,MIHOMO-Apple-Intelligence,COMPUTER_AI" in rules
    assert "RULE-SET,MIHOMO-Telegram,COMPUTER_SOCIAL" in rules
    assert "RULE-SET,MIHOMO-Crypto,COMPUTER_DEFAULT" in rules
    assert "RULE-SET,MIHOMO-APNs,DIRECT" in rules
    assert rules.index("DOMAIN-SUFFIX,openai.com,COMPUTER_AI") < rules.index(
        "RULE-SET,MIHOMO-Apple-Intelligence,COMPUTER_AI"
    )
    assert rules[-2:] == ["GEOIP,CN,DIRECT", "MATCH,PROXY"]
    assert len(rules) == 43
